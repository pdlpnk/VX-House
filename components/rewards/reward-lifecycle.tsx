...
export function RewardLifecycle({ reward }: { reward: RewardView }) {
  ...
  async function claim() {
    ...
    try {
      ...
      setMessage("Reward предоставлен. История обновлена сервером."); router.refresh();
  ...
  return <>
    ...
    <section className={styles.rewardStatePanel} aria-live="polite">
      <header className={styles.rewardStateHeader}><span><Gift aria-hidden="true" /></span><div><small>Серверное состояние</small><h2>{rewardStatusLabel(reward.status)}</h2><p>Клиент не может самостоятельно изменить этот статус.</p></div><RewardStatusPill status={reward.status} /></header>
      <div className={styles.rewardStateGrid}>
        <Card className={styles.rewardStateCard}>
          ...
          <div className={styles.sourceDecisionNotice}><Info aria-hidden="true" /><p><strong>Источник истины — сервер.</strong> Получение создаёт отдельный переход статуса и не изменяет VX Points.</p></div>
        </Card>
