...
export function RewardHistory({ rewards, catalogHref }: { rewards: RewardView[]; catalogHref: string }) {
  ...
  return <DashboardPage><DashboardHeading eyebrow="VX Rewards" title="История Rewards" description="Полная хронология оснований, решений и предоставленных преимуществ." action={<Button asChild variant="outline"><Link href={catalogHref}><ArrowLeft aria-hidden="true" />К преимуществам</Link></Button>} /><div className={styles.economyDisclosure} role="note"><Info aria-hidden="true" /><p><strong>История отделена от VX Points.</strong> Каждый переход статуса содержит причину и время.</p></div>{events.length === 0 ? <Card className={styles.rewardHistoryEmpty}><span><FileClock aria-hidden="true" /></span><small>История пока пуста</small><h2>Событий VX Rewards пока нет</h2><p>История появится после назначения первого преимущества.</p></Card> : <div className={styles.economyEntityList}>{events.map((event) => <article key={event.id}><span><CircleDashed aria-hidden="true" /></span><div><small>{new Intl.DateTimeFormat("ru", { dateStyle: "medium", timeStyle: "short" }).format(new Date(event.occurredAt))}</small><h3>{event.rewardTitle}</h3><p>{event.reason}</p><RewardStatusPill status={event.toStatus} /></div></article>)}</div>}</DashboardPage>;
}
