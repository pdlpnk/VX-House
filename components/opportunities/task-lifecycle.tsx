...
const statusCopy: Record<UserTaskView["status"], { label: string; next: string; tone: "neutral" | "brand" | "attention" | "success" }> = {
  AVAILABLE: { label: "Доступно", next: "Откройте задание и прочитайте инструкцию.", tone: "brand" }, ACCEPTED: { label: "Принято", next: "Выполните шаги инструкции и отметьте задание завершённым.", tone: "brand" }, IN_PROGRESS: { label: "Выполняется", next: "Когда закончите, отправьте задание менеджеру.", tone: "attention" }, AWAITING_SUBMISSION: { label: "Ожидает отправки", next: "Проверьте данные и отправьте результат.", tone: "attention" }, SUBMITTED: { label: "Отправлено", next: "Результат передан менеджеру.", tone: "brand" }, UNDER_REVIEW: { label: "Ожидает проверки", next: "Мы сообщим, когда проверка завершится.", tone: "attention" }, CLARIFICATION_REQUIRED: { label: "Требуется уточнение", next: "Изучите комментарий менеджера.", tone: "attention" }, RESUBMISSION_REQUIRED: { label: "Нужна повторная отправка", next: "Подготовьте исправленную версию результата.", tone: "attention" }, CONFIRMED: { label: "Подтверждено", next: "Следующее задание уже доступно.", tone: "success" }, REJECTED: { label: "Отклонено", next: "Изучите основание решения.", tone: "neutral" }, EXPIRED: { label: "Срок истёк", next: "Действия по заданию недоступны.", tone: "neutral" }, CANCELLED: { label: "Отменено", next: "Действия по заданию недоступны.", tone: "neutral" },
};
...
export function TaskLifecycle({ initialTask }: { initialTask: UserTaskView }) {
  ...
  async function command(action: "start" | "complete" | "draft" | "submit") {
    ...
    try {
      ...
      setTask(result); setMessage(action === "draft" ? "Черновик сохранён." : action === "submit" || action === "complete" ? "Задание отправлено менеджеру и ожидает проверки." : "Выполнение начато.");
  ...
  return <>
    <section className={styles.lifecycleExplorer} aria-labelledby="lifecycle-title"><div className={styles.lifecycleExplorerHeader}><div><span>Выполнение задания</span><h2 id="lifecycle-title">Ваш текущий этап</h2><p>Следуйте инструкции — статус обновится автоматически после каждого действия.</p></div><StatusPill tone={state.tone}>{state.label}</StatusPill></div></section>
    <motion.section className={styles.lifecycleStatePanel} initial={shouldReduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: shouldReduceMotion ? 0 : 0.28 }} aria-live="polite">
      ...
      <div className={styles.lifecycleStateGrid}><Card className={styles.lifecyclePrimaryCard}><div className={styles.stateCardHeading}><ListChecks aria-hidden="true" /><div><small>Инструкция</small><h3>{task.task.instruction?.title ?? "Инструкция не назначена"}</h3></div></div>{task.task.instruction?.sections.map((section) => <section key={section.id} className={styles.opportunityNextStep}><span>{section.title}</span><p>{section.body}</p></section>)}<ol className={styles.lifecycleStepList}>{task.task.instruction?.steps.map((step) => <li key={step.id}><span>{step.position}</span><div><strong>{step.title}</strong><p>{step.body}</p></div><StatusPill tone={step.required ? "brand" : "neutral"}>{step.required ? "Обязательно" : "Дополнительно"}</StatusPill></li>)}</ol></Card><div className={styles.lifecycleAsideCards}><Card className={styles.nextStepCard}><span><ShieldCheck aria-hidden="true" /></span><div><small>Что дальше</small><h3>Следующий шаг</h3><p>{state.next}</p></div></Card>{["ACCEPTED", "IN_PROGRESS"].includes(task.status) ? <Button onClick={() => command("complete")} disabled={pending}>Я выполнил задание <Send aria-hidden="true" /></Button> : null}</div></div>
    ...
    <section className={styles.taskHistoryPreview} aria-labelledby="task-history-title"><div className={styles.taskHistoryHeading}><span><History aria-hidden="true" /></span><div><small>История задания</small><h2 id="task-history-title">Что уже произошло</h2><p>Здесь сохраняются все этапы выполнения.</p></div></div>{task.history.map((event) => <div className={styles.taskHistoryEmpty} key={event.id}><Clock3 aria-hidden="true" /><div><strong>{statusCopy[event.toStatus].label}</strong><p>{event.reason} · {new Date(event.occurredAt).toLocaleString("ru-RU")}</p></div></div>)}</section>
  </>;
}
