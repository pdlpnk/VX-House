...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await limitRequest({ namespace: "tasks.submit", key: `${principal.userId}:${deriveNetworkIdentifier(request)}`, limit: 15, windowSeconds: 3600 });
    const body = await readJsonBody(request);
    return json(await getOpportunityTaskService().submit(principal, (await params).id, body.payload, String(body.idempotencyKey ?? "")));
