...
| `registration_started` | Первый выбор роли; если клиентское событие потерялось — гарантированно при серверном создании аккаунта | Клиент + серверный fallback | Не отправляется |
| `email_confirmed` | Только после успешной проверки одноразового кода в серверной транзакции | Сервер | `lead` |
